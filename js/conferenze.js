...
export async function apriPratica(id) {
  ...
  apriDrawer(`Conferenza n° ${p.progressivo ?? `m${p.id}`} — ${p.ragione_sociale || ''}`, '', `
    ...
    ${['approvata', 'respinta'].includes(p.aut_stato) ? `
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        ${state.soloDirettore ? '' : `<button class="btn btn-primary" id="cf-lettera">📨 Lettera di incarico al tecnico</button>`}
  ...
  $('#cf-salva').addEventListener('click', async (ev) => {
    ...
    if (p.incarico_id && $('#cf-tecnico').value) {
...
async function protocollaIn(p) {
  ...
  mod.apriForm('IN', {
    ...
    if (p.fonte === 'modulo') {
      ...
      await depositaRiepilogo('conf', p, nuovo, PERCORSO_VAULT);
    }
  });
  toast(p.fonte === 'modulo'
    ? 'Maschera IN precompilata: salva e il riepilogo PDF del modulo nasce da solo, col numero nel nome. Se hai un documento originale (PEC, lettera) allegalo pure: resta lui il principale.'
    : 'Maschera IN precompilata: allega il documento arrivato (PEC, lettera) e salva.', 'ok');
}
