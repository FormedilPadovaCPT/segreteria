/* ============================================================
   ...
   Quattro schede (dal 26/09/2026 MESE resta qui con la vista e lo stato
   comune; FATTURE, MANDATI e PRESTAZIONI stanno in
   fatture-tecnici-fatture.js, -mandati.js, -prestazioni.js):
...
import { renderFatture, formFattura, dettaglioFattura } from './fatture-tecnici-fatture.js';
import { renderMandati } from './fatture-tecnici-mandati.js';
import { renderPrestazioni } from './fatture-tecnici-prestazioni.js';
...
let incarichiMese = [];
/* filtroTec e annoPrest li cambiano anche le schede Fatture e Prestazioni,
   che stanno in file loro: una variabile importata non si può riassegnare
   da fuori, quindi passano da qui. */
const impostaFiltroTec = (v) => { filtroTec = v; };
const impostaAnnoPrest = (v) => { annoPrest = v; };
...
/* ciò che serve alle schede Fatture, Mandati e Prestazioni. `tecnici` e
   `conf` le riassegna solo caricaBase(), qui: di là si vede sempre il
   valore corrente (binding vivo, sola lettura). dettaglioFattura resta
   esportata anche da qui, come prima. */
export { CARTELLA_FATTURE, TIPO_DOC_FATTURA, STATI_FATT, tecnici, conf, filtroTec, annoPrest,
  impostaFiltroTec, impostaAnnoPrest, nomeTec, pill, b64, fiscDi,
  dettaglioIncarico, tabellaPrestazioni, avvisaCoordinatore, dettaglioFattura };
