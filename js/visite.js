  ...
  host.innerHTML = `
    ...
    <div class="table-wrap">
      <table class="tbl">
        ...
        <tbody>${righe || '<tr><td colspan="9" class="empty">Nessuna richiesta con questo filtro.</td></tr>'}</tbody>
      </table>
    </div>
...
function nuovaRichiesta() {
  ...
  apriDrawer('Nuova richiesta di visita (manuale)', '', `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
      ...
      <div class="field"><label>Tipo</label>
        <select id="nv-tipo"><option value="visita">Visita singola</option><option value="serie">Serie di visite</option></select></div>
    </div>
    ...
    <div class="field"><label>Note (oggetto della visita, preferenze sul tecnico…)</label><textarea id="nv-note" rows="3"></textarea></div>
  ...
  $('#nv-crea').addEventListener('click', async (ev) => {
    ...
    const { data: nuova, error } = await sb.from('s_visite_richieste').insert({
      ...
      tab_origine: $('#nv-tipo').value,
      tipo_richiesta: $('#nv-tipo').value,
  ...
  apriDrawer(`Richiesta n° ${p.progressivo ?? `m${p.id}`} — ${p.ragione_sociale || ''}`, '', `
    ...
    <div class="dt-quadro-riga">
      ...
      <span class="dt-quadro-stato">${p.tecnico_assegnato
        ...
        : p.tecnico_proposto ? `proposto dalla zona: ${esc(nomeTecnico(p.tecnico_proposto))} — da confermare` : 'da assegnare'}</span>
    </div>
    ${p.protocollo_in_id ? `
    <div class="dt-quadro-riga">
    ...
    ${campo('Tipo', p.tipo_richiesta === 'serie' ? `Serie di visite (${cc.length} cantieri)` : `Visita singola${p.tipo_visita ? ` — ${p.tipo_visita}` : ''}`)}
...
function campiVisita(p) {
  ...
  return [
    ...
    ['TipologiaRichiesta', p.tipo_richiesta === 'serie' ? 'Richiesta Serie di Visite' : 'Richiesta Visita da Impresa'],
