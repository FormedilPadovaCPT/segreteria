  ...
  apriDrawer(`Richiesta n° ${p.progressivo ?? `m${p.id}`} — ${p.ragione_sociale || ''}`, '', `
    ...
    ${['approvata', 'respinta'].includes(p.aut_stato) ? `
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        ${state.soloDirettore ? '' : `<button class="btn btn-primary" id="vs-lettera">📨 Lettera di incarico al tecnico</button>`}
...
async function protocollaIn(p) {
  ...
  mod.apriForm('IN', {
    ...
    if (p.fonte === 'modulo') {
      ...
      await depositaRiepilogo('vis', p, nuovo, PERCORSO_VAULT);
    }
  });
